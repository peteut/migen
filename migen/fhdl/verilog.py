...
def _printconstant(node):
    if node.signed:
        ...
        return "{}'sd{}".format(node.nbits, val), True
    else:
    ...
    for sig in sorted(sigs - ios, key=hash):
        ...
        if sig in wires:
            ...
            if sig not in comb_regs:
                r += "reg {} = {};\n".format(
                    _printsig(ns, sig), _printexpr(ns, sig.reset)[0])
            else:
    ...
    if f.comb:
        ...
        for n, g in enumerate(groups):
            if len(g[1]) == 1 and isinstance(g[1][0], _Assign):
                ...
                r += "\n" + syn_off
                r += "reg {};\n".format(_printsig(ns, dummy_d))
                r += syn_on

                r += "always @(*) begin\n"
                if display_run:
                    r += "\t$display(\"Running comb block #{}\");\n".format(n)
                for t in sorted(g[0], key=hash):
                    r += "\ṭ{} <= ̣{};\n".format(ns.get_name(t), _printexpr(ns, t.reset)[0])
                r += _printnode(ns, _AT_NONBLOCKING, 1, g[1])

                r += syn_off
                r += "\t{} <= ̣{};\n".format(ns.get_name(dummy_d), ns.get_name(dummy_s))
                r += syn_on
                r += "end\n"
    r += "\n"
    return r
...
def _printsync(f, ns):
    ...
    for k, v in sorted(f.sync.items(), key=itemgetter(0)):
        r += "always @(posedge {}) begin\n".format(ns.get_name(f.clock_domains[k].clk))
